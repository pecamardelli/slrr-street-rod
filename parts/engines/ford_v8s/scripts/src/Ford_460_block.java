...
public class Ford_460_block extends Block_Vee_OHV
{
	public Ford_460_block( int id )
	{
		super( id );
		name = "Ford 460 V8 Block";
		description = "";

		brand_new_prestige_value = 59.86;
		value = tHUF2USD(440);
		...
		crank_center_to_cylinder_top = 287;
	...
	public void updatevariables()
	{
		...
		if (the_car)
		{
			...
			if (tab = the_car.getSfxTable(2))
			{
				...
				tab.addItem(new ResourceRef(parts.engines.ford_v8s:0x00003818r), 4000.0, 1400.0, 6500.0, 2.7, 2.9);		// 2000	RPM
				tab.addItem(new ResourceRef(parts.engines.ford_v8s:0x00003830r), 5400.0, 4300.0, 10000.0, 2.5, 2.8);	// 4000	RPM
	...
	public void addStockParts( Descriptor desc )
	{
		...
		addPart( parts.engines.ford_v8s:0x000040FEr, "Ford_460_crankshaft", desc );
		addPart( parts.engines.ford_v8s:0x000000BBr, "Ford_stock_crankshaft_timing_gear", desc );
		addPart( parts.engines.ford_v8s:0x00004055r, "Ford_460_connecting_rods", desc );
		addPart( parts.engines.ford_v8s:0x00004056r, "Ford_460_pistons", desc );
		
		number = random();
		
		if( number < 0.65 )
		{
			addPart( parts.engines.ford_v8s:0x000040B4r, "Ford_460_camshaft", desc );
		}
		else
		{
			number2 = random();
		...
		addPart( parts.engines.ford_v8s:0x000040B5r, "Ford_460_timing_cover", desc );
		addPart( parts.engines.ford_v8s:0x00004095r, "Ford_460_oil_pan", desc );
		...
		addPart( parts.engines.ford_v8s:0x00004083r, "Ford_460_flywheel", desc );
		addPart( parts.engines.ford_v8s:0x00004084r, "Ford_460_clutch", desc );
		addPart( parts.engines.ford_v8s:0x0000409Br, "Ford_460_pulley", desc );
		addPart( parts.engines.ford_v8s:0x00004052r, "Ford_460_water_pomp", desc );
		
		number = random();
		
		if( number < 0.73 )
		{
			addPart( parts.engines.ford_v8s:0x000040ADr, "Ford_460_left_cylinder_head", desc );
			addPart( parts.engines.ford_v8s:0x000040B0r, "Ford_460_left_exhaust_header", desc );
			addPart( parts.engines.ford_v8s:0x000040AFr, "Ford_460_right_cylinder_head", desc );
			addPart( parts.engines.ford_v8s:0x000040B2r, "Ford_460_right_exhaust_header", desc );
			addPart( parts.engines.ford_v8s:0x00004042r, "Ford_460_left_cylinder_head_cover", desc );
			addPart( parts.engines.ford_v8s:0x00004044r, "Ford_460_right_cylinder_head_cover", desc );
		}
		else
		{
			...
			addPart( parts.engines.ford_v8s:0x00000542r, "Ford_racing_left_cylinder_head_cover", desc );
			addPart( parts.engines.ford_v8s:0x00000544r, "Ford_racing_right_cylinder_head_cover", desc );
		}
		
		addPart( parts.engines.ford_v8s:0x00003048r, "block_cover", desc );
		
		number = random();
		
		if( number < 0.42 )
		{
			addPart( parts.engines.ford_v8s:0x000040BEr, "Ford_460_4brl_intake_manifold", desc );
		}
		else if( number < 0.67 )
		{
		...
		else
		{
			...
			if( number2 < 0.4 )
			{
				addPart( parts.engines.ford_v8s:0x00005686r, "Ford_460_4brl_air_filter", desc );
			}
			else if( number2 < 0.63 )
			{
				addPart( parts.engines.ford_v8s:0x00000886r, "Edelbrock_4brl_air_filter", desc );
			}
			else if( number2 < 0.83 )
			{
