...
{
	final float bore_difference = 2.5; // in milimeters //
	final float ccct_difference = 5.0; // in milimeters //
	...
	float	def_crank_center_to_cylinder_top = 0.0; // in milimeters //
...
//			System.log("!!!Crankshaft required on slot!!!");
	...
	public Part getCrankshaftBearing()
	{
		...
		if (res && res instanceof SlidingEnginePart)
			return res;
//		else
//			System.log("!!!Crankshaft bearnig bridge required on slot!!!");
...
*/	}
	...
	{
		def_bore = bore;
		def_crank_center_to_cylinder_top = crank_center_to_cylinder_top;
		defaults_was_set = 1;
	}
	...
	public void load( File saveGame )
	{
		...
		if (save_ver >= 4)
		{
			bore = saveGame.readFloat();
			crank_center_to_cylinder_top = saveGame.readFloat();
		}
	}
	...
	public void save( File saveGame )
	{
		...
		{
			saveGame.write( bore );
			saveGame.write( crank_center_to_cylinder_top );
		}
	}
	...
	public void buildTuningMenu( Menu m )
	{
		...
		old_bore = bore;
		old_ccct = crank_center_to_cylinder_top;
		...
		float min_bore = def_bore;
		float max_bore = def_bore + bore_difference + addition;
		float max_ccct = def_crank_center_to_cylinder_top + addition;
		float min_ccct = def_crank_center_to_cylinder_top - ccct_difference - addition;

		m.addItem( "Bore", 3, bore, min_bore, max_bore, (max_bore - min_bore) * 10 + 1, null ).printValue( "   %1.1f mm" );
		m.addItem( "Deck height", 4, crank_center_to_cylinder_top, min_ccct, max_ccct, (max_ccct - min_ccct) * 10 + 1, null ).printValue( "   %1.1f mm" );
	...
	public void endTuningSession( int cancelled )
	{
		if( cancelled )
		{
			...
			bore = old_bore;
			crank_center_to_cylinder_top = old_ccct;
		}
		else
		{
			...
			if (old_ccct != crank_center_to_cylinder_top)
				GameLogic.spendTime(64*60);
	...
	public void handleMessage( Event m )
	{
		...
		{
			bore = def_bore;
			m.gadget.osd.findGadget( this, 3 ).setValue( bore );
			crank_center_to_cylinder_top = def_crank_center_to_cylinder_top;
			m.gadget.osd.findGadget( this, 4 ).setValue( crank_center_to_cylinder_top );
		}
		else
		if( m.cmd == 1 )
		{
		...
		if( m.cmd == 3 )
		{
			bore = ((Slider)m.gadget).value;
			((Slider)m.gadget).setValue( bore );
		}
		else
		if( m.cmd == 4 )
		{
			crank_center_to_cylinder_top = ((Slider)m.gadget).value;
			((Slider)m.gadget).setValue( crank_center_to_cylinder_top );
		}
	}
	//---------tuning
}
