...
//				posStart = map.getNearestCross( posStart );
			...
			{	//day time settings
				...
				map.addTraffic( new GameRef(cars.fake_racers.Badge:0x00000006r), 80, 2, 5, 2 );
				map.addTraffic( new GameRef(cars.fake_racers.Ninja:0x00000006r), 20, 2, 5, 2 );
				map.addTraffic( new GameRef(cars.fake_racers.Stallion:0x00000006r), 12, 2, 5, 2 );
				map.addTraffic( new GameRef(cars.fake_racers.Sunset:0x00000006r), 30, 2, 10, 2 );
				map.addTraffic( new GameRef(cars.fake_racers.Teg:0x00000006r), 55, 2, 10, 2 );
				...
				if( GameLogic.gameMode == GameLogic.GM_CARREER )
				{
					...
					tDayRacers = Config.dayRacers;
					if( tDayRacers == 1 ) addDayRacers();
				} else
			...
			{	//night time settings
				...
				if( GameLogic.gameMode == GameLogic.GM_CARREER )
				{
					...
					tNightRacers = Config.nightRacers;
					if( tNightRacers == 1 ) prepareNightRace();
				} else
				if( GameLogic.gameMode == GameLogic.GM_DEMO )
				{
